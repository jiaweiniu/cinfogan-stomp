...
class Cgan_GANUpdater(training.StandardUpdater):
    def __init__(self, iterator, noise_iterator, noise_dim,
        ...
        super(Cgan_GANUpdater,self).__init__(iterators, optimizers, device=device)
    ...
    def update_core(self):
        if self.epoch==self.epoch_counter:
            ...
            if self.collision_measure!=0:
                ...
                f=open('results/f_cgan_metric.dat','a')
        ...
        y_fake, y_real = self.forward()
        losses = self.backward(y_fake, y_real)
        self.update_params(losses, report=True)


class Cgan_WassersteinGANUpdater(training.StandardUpdater):
    def __init__(self, iterator, noise_iterator, noise_dim,
                 x_dim, xi_dim, experiment,
                 optimizer_generator,
        ...
        super(Cgan_WassersteinGANUpdater,self).__init__(iterators, optimizers, device=device)
        self.noise_dim=noise_dim
        self.epoch_counter=0
        self.x_dim = x_dim
        self.xi_dim = xi_dim
